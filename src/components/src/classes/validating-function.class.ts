export class ValidatingFunctions {
	...
	static cpf(value: any) {
		return ValidatingFunctions.notNull(value) &&
			ValidatingFunctions.validateCPF(value) &&
			value.split('').length === 11
	}

	static cnpj(value: any) {
		return ValidatingFunctions.notNull(value) &&
			ValidatingFunctions.validateCNPJ(value) &&
